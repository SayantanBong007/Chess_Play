...
function LiveChessGame({
  chessGameDetails,
}: {
  chessGameDetails: ChessGameDetailsInterface;
}) {
  const {
    boardOrientation,
    myAddress,
  ...
  const [opponentWalletAddress, setOpponentWalletAddress] =
    useState(opponentAddress);
    ...
    if (game.game_over()) {
      ...
      setMatchEndData({
        ...
        matchResult: isDraw
          ...
          : amIWinner && isMatchCreator
          ? MatchResultEnum.MATCH_CREATOR
          : MatchResultEnum.MATCH_JOINER,
      });
    }
  }
  useEffect(() => {
  ...
  function connectWithOpponent() {
    if (isMatchCreator) {
      ...
      socket.on(
        "connection-req-from-player",
        (_matchId: string, _opponentAddress: string) => {
      ...
      socket.on(
        "connection-req-accepted",
        (_matchId: string, _yourAddress: string) => {
    ...
    <>
        ...
        <div className="flex items-stretch gap-10" ref={chessBoardDivRef}>
          <div className="basis-7/12 h-full">
            <StyledChessBoard
              ...
              onDrop={onDrop}
              myAddress={myAddress}
              opponentAddress={opponentWalletAddress}
              isCheck={isCheck}
            />
          </div>
      ...
      <LoadingModel isOpen={matchEndData?.matchOver}>
        <MatchResultPopup
          ...
          chessBoardDivRef={chessBoardDivRef}
          pgn={game.pgn()}
          movesHistory={gameHistroy}
          myAddress={myAddress}
          opponentAddress={opponentWalletAddress}
        />
      </LoadingModel>
