...
function JoinMatch() {
  ...
  async function joinMatchHandler() {
    ...
    const response: AxiosResponse<MatchDataResponse> | undefined =
      await joinMatchApiCall(matchId, address);

    if (Number(response?.status) < 400 && response?.data?.data) {
      const _matchId = response?.data?.data?.matchId;
      const matchJoined = await joinMatch(
        _matchId,
        Number(matchDetails?.stackedAmount)
      );
      if (matchJoined) {
        router.push(`/match/${_matchId}?id=2`);
      } else {
